...
def inset_chelicerae_support_loop(node: hou.SopNode) -> None:
    ...
    support_prims = get_chelicerae_support_strip_prims(geo)
    ...
    adjust_chelicerae_support_loop(node)
    deduplicate_id_attr(geo, None, keep_first=True)


def get_chelicerae_support_strip_prims(geo: hou.Geometry) -> list[hou.Prim]:
...
def adjust_chelicerae_support_loop(node: hou.SopNode) -> None:
    geo: hou.Geometry = node.geometry()
    ...
    adjust_right_chelicerae_support_points(by_id)
    adjust_right_chelicerae_membrane_points(by_id)
    adjust_left_chelicerae_support_points(by_id)
    adjust_bottom_chelicerae_support_points(by_id)


def adjust_right_chelicerae_support_points(points_by_id: dict[str, set[hou.Point]]) -> None:
    ...
    for support_id, mid_id in sections:
        inset_support = get_inset_chelicerae_support_point(points_by_id, support_id)
        inset_mid = get_inset_chelicerae_support_point(points_by_id, mid_id)
        inset_support.setPosition((inset_support.position() + inset_mid.position()) * 0.5)


def adjust_right_chelicerae_membrane_points(points_by_id: dict[str, set[hou.Point]]) -> None:
    ...
    inset_support3 = get_inset_chelicerae_support_point(points_by_id, cheliceraestartmembranesupport(3))
    inset_membrane4 = get_inset_chelicerae_support_point(points_by_id, cheliceraemembrane(4))
    inset_start3 = get_inset_chelicerae_support_point(points_by_id, cheliceraestart(3))

    support3 = get_original_chelicerae_point(points_by_id, cheliceraestartmembranesupport(3))
    support_uppermid = get_original_chelicerae_point(points_by_id, upper_middle(cheliceraestartmembranesupport, 1))
...
def adjust_left_chelicerae_support_points(points_by_id: dict[str, set[hou.Point]]) -> None:
...
def adjust_bottom_chelicerae_support_points(points_by_id: dict[str, set[hou.Point]]) -> None:
    sections = (
    ...
    for end_id, section_id in sections:
        inset_end = get_inset_chelicerae_support_point(points_by_id, end_id)
        inset_section = get_inset_chelicerae_support_point(points_by_id, section_id)
        inset_end.setPosition((inset_end.position() + inset_section.position()) * 0.5)


def get_original_chelicerae_point(points_by_id: dict[str, set[hou.Point]], pt_id: str) -> hou.Point:
    return min(points_by_id[pt_id], key=lambda pt: pt.number())


def get_inset_chelicerae_support_point(points_by_id: dict[str, set[hou.Point]], pt_id: str) -> hou.Point:
...
def add_start_membrane_support_loops(node: hou.SopNode) -> None:
    ...
    added = add_support_loop(geo, ratio, membrane, support)
    add_support_loop(geo, ratio, support, added[0])
...
def add_start_section_support_loops(node: hou.SopNode) -> None:
    ...
    add_support_loop(geo, ratio, start, middle)


def add_support_loop(
    geo: hou.Geometry,
    ratio: float,
